...
var left = vis.append("g").attr("id", "left")

left.append("g").attr("class", "links")

left.append("g").attr("class", "nodes")

var right = vis.append("g").attr("id", "right")

right.append("g").attr("class", "links")

right.append("g").attr("class", "nodes")
  ...
  d3.json(nodes_json, function(json) {
    ...
    json.nodes.forEach(function(d, i) {
      ...
      if (n) {
        ...
        var n = json.nodes[i]
        if (n.flags.gateway) n.offset = -Math.random()*30
        else if (n.flags.client) n.offset = +Math.random()*5
        else n.offset = +Math.random()*20
      }
    })
